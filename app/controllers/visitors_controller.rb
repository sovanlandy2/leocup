class VisitorsController < ApplicationController
	def index
		...
		@winning_teams = @winning_teams_raw.to_a.each_slice(4).to_a
		@total_votes = TeamVote.where(tournament_id: @current_tournament).count
		if current_user.present?
			@current_voted_teams = TeamVote.where(user_id: current_user.id,tournament_id: @current_tournament).pluck(:team_id)
		end
